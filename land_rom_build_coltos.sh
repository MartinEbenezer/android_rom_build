echo $(date '+%Y-%m-%d %H:%M:%S') ​"Begin Colt OS Build for Land"
...
WORK_DIR=colt_land
ROM_LINK=https://github.com/Colt-Enigma/platform_manifest
ROM_BRANCH=c11
...
echo $(date '+%Y-%m-%d %H:%M:%S') ​"Completed Colt OS Build for Land"

echo $(date '+%Y-%m-%d %H:%M:%S') ​"Upload ROM : Begin"

cd $OUT
ROM_ZIP_FILE=`find . -name *fficial*zip | head -1`
